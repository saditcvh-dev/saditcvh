...
export class TabsNavigationComponent {
  ...
  @Input() activeTab!: 'preview' | 'metadata' | 'security' | 'notes' | 'history';
  ...
  shouldShowTab(tabId: string): boolean {
    ...
    return tab.showFor.includes(this.selectedNode.type);
  }

  onTabClick(tabId: 'preview' | 'metadata' | 'security' | 'notes' | 'history'): void {
